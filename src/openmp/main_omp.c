...
static void fill_matrix(Matrix *matrix, const char *seq_a, const char *seq_b)
{
    ...
    /*
     * Rows and columns cannot be parallelized directly because F[i][j] reads
     * F[i-1][j] and F[i][j-1]. In row-major or column-major parallel loops,
     * some cells would read neighbors that have not been computed yet.
     *
     * Anti-diagonals work because every dependency of cell (i, j) is on an
     * earlier anti-diagonal:
     *   (i-1, j-1) has sum i+j-2
     *   (i-1, j)   has sum i+j-1
     *   (i, j-1)   has sum i+j-1
     * Cells with the same i+j never depend on each other.
     */
    for (diagonal = 2; diagonal <= len_a + len_b; diagonal++) {
        ...
        cell_count = (long)(last_i - first_i + 1);

        /*
         * OpenMP shares matrix, seq_a, seq_b, first_i, and diagonal across
         * threads. The loop counter k and derived i/j values are private to
         * each iteration, so every thread writes a different matrix cell.
         *
         * schedule(static) gives each thread a fixed contiguous block, which is
         * simple and low overhead for equal-cost DP cells. schedule(dynamic)
         * or schedule(guided) can help irregular work, but here they add extra
         * scheduling cost without changing the amount of work per cell.
         *
         * The implicit barrier at the end of parallel for is required: diagonal
         * d + 1 must not start until all cells from diagonal d are complete.
         */
#pragma omp parallel for schedule(static) default(none) shared(matrix, seq_a, seq_b, first_i, diagonal, cell_count) private(k)
        for (k = 0; k < cell_count; k++) {
            size_t i = first_i + (size_t)k;
            size_t j = diagonal - i;
            Direction direction;
